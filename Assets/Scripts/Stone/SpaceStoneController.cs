...
public enum SpaceStoneState { Settled, Dragging, Dropping }   // ★ Dropping 추가
                                   ...
                                   IDragHandler
{
    ...
    /* ───────────── 캐시 / 상태 ───────────── */
    public SpaceStoneState State { get; private set; } = SpaceStoneState.Settled;
    ...
    void Awake()
    {
        ...
        BuildColliders();
        SetSettledVisual();
        if (State == SpaceStoneState.Settled)
            tag = "PlacedStone";
    }
    #endregion
    /* ==================================================================== */
    ...
    void FixedUpdate()
    {
        if (isRotating && State == SpaceStoneState.Dragging)
    ...
    {
        /* ① 드래그 중 ‘가만히 있음’ → 회전 전환 -------------------------- */
        if (State == SpaceStoneState.Dragging && !isRotating && activePointer != -1)
        {
        ...
        if (State == SpaceStoneState.Dropping)
        {
    ...
    {
        if (activePointer != -1 || State != SpaceStoneState.Settled) return;
    ...
    void StartDropping()
    {
        State = SpaceStoneState.Dropping;
