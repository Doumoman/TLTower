...
{
    /* ────── 싱글톤 기본 ────── */
    public static NewAnimationManager Instance { get; private set; }
    void Awake()
    {
    ...
    public void AfterTicks()
    {
        ...
        handler = (sender, args) =>
        {
            if (TickManager.Instance.tickCount % 4 == 0)
            {
                TickManager.Instance.OnTickEvent -= handler;
    ...
    void OnAllStonesSnapped()
    {
        ...
        AfterTicks(); // 마지막 돌이 끼워지면 해당 로직 실행
        Debug.Log("우주애니메이션 실행");
    }
