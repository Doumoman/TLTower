...
{
    /* ────── 싱글톤 기본 ────── */
    public static AnimationManagerTest Instance { get; private set; }
    void Awake()
    {
    ...
    {
        Debug.Log("우주 돌 애니메이션 테스트 시작!");
        SpawnSpaceStones();
    }

    // 테스트용: 수동으로 돌 스냅 시뮬레이션
    public void OnTestSnapButtonClicked()
    {
        Debug.Log("돌 스냅 테스트!");
        NotifyStoneSnapped();
    }
    ...
    // 테스트용: 모든 돌을 한번에 스냅
    public void OnTestAllSnapButtonClicked()
    {
        Debug.Log("모든 돌 스냅 테스트!");
