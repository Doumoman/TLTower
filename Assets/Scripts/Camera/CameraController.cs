...
public class CameraController : MonoBehaviour
{
    ...
    private float TopLimit
    {
        get
        {
            ...
            if (cm.chapter.ToString().Contains("autumn"))
            {
                ...
                limit = ChapterManager.CloudCheckPoint[(int)cm.chapter - (int)chapter.autumn + 1].transform.position.y - 3;
            }
    ...
    IEnumerator CoRaiseCameraY(float amount, float dur)
    {
        ...
        while (_centerRoutine != null)
            yield return null;
        if(AnimationManager.Instance != null) AnimationManager.Instance.SpawnSpaceStones();
        if(NewAnimationManager.Instance !=null) NewAnimationManager.Instance.SpawnSpaceStones();
        _raiseRoutine = null;
    }
    #endregion
